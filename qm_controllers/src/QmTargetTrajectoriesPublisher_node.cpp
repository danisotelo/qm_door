...
// Subscriber callback to modify the FIXED_BASE variable
void fixedBaseCallback(const std_msgs::Bool::ConstPtr& msg){
    FIXED_BASE = msg->data;
    ROS_INFO("Received FIXED_BASE status: %s", FIXED_BASE ? "true" : "false");
}
                                              ...
                                              const SystemObservation& eeState)
{
    ...
    const vector_t BaseTargetPose = [&]() {
        ...
        target(3) = 0; //target(3) = BaseCurrenPose(3) + cmdVel(3) * timeToTarget;  
        target(4) = 0;
        target(5) = 0;
        return target;
    }();
...
TargetTrajectories EeCmdVelToTargetTrajectories(const vector_t& cmdVel,
    ...
    const vector_t BaseTargetPose = [&](){
        ...
        target(0) = EeTargetPose(0) - ARM_DIST;// target(0) = EeTargetPose(0) - ARM_DIST * std::cos(BaseCurrenPose(3)); // 
        if (FIXED_BASE) {
            target(1) = 0;
        } else {
            target(1) = EeTargetPose(1); // target(1) = EeTargetPose(1) - ARM_DIST * std::sin(BaseCurrenPose(3));
        }
        target(2) = COM_HEIGHT + FEET_HEIGHT;
        target(3) = 0; // yaw; 
        target(4) = 0;
...
TargetTrajectories EEgoalPoseToTargetTrajectories(const Eigen::Vector3d& position, const Eigen::Quaterniond& orientation,
    ...
    const vector_t BaseTargetPose = [&](){
        ...
        target.setZero();
        target = BaseCurrenPose;
        target(0) = position(0) - ARM_DIST; //target(0) = position(0) - ARM_DIST * std::cos(yaw); 
        if (FIXED_BASE) {
            target(1) = 0;
        } else {
            target(1) = position(1); // target(1) = position(1) - ARM_DIST * std::sin(yaw);
        }
        target(2) = COM_HEIGHT + FEET_HEIGHT;
        target(3) = 0; //yaw; 
        target(4) = 0;
...
int main(int argc, char* argv[]) {
    ...
    // Add a subscriber for the FIXED_BASE boolean status
    ros::Subscriber fixedBaseSubscriber = nodeHandle.subscribe(
        "/fixed_base", 10, fixedBaseCallback);
