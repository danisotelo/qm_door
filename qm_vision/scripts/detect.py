    ...
    def process_depth(self):
        ...
        if mask is not None and depth_image is not None:
            if mask.shape != depth_image.shape:
                ...
                rospy.loginfo("Mask resized to match depth image dimensions.")

            # Apply the mask to the depth image
            masked_depth = np.where(mask > 0, depth_image, 0)
            rospy.loginfo("Mask applied to depth image.")
            ...
            for u in range(0, width, downsample_factor):
                ...
                if depth_value > 0:
                    if rospy.get_param("~optical_frame") == "camera_link_optical":
                        rospy.loginfo(rospy.get_param("~optical_frame") == "camera_link_optical")
                        z = depth_value
                    else:
                        z = depth_value / 1000
